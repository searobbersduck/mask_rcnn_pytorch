...
class DRDetectionDS_xml(Dataset):
    def __init__(self, root, ann_root, scale_size=None):
        ...
        for index in self.data_list:
            anns, bbox = self.__read_xml(index)
    ...
    def __getitem__(self, item):
        ...
        return img, anns, image_path, self.bboxs_list[item]
...
def cls_train(train_data_loader, model, criterion, optimizer, epoch, display):
    ...
    logger = []
    for num_iter, (images, anns, image_paths, bboxs) in enumerate(train_data_loader):
...
def cls_eval(val_data_loader, model, criterion, display):
    ...
    for num_iter, (images, anns, image_paths, bboxs) in enumerate(val_data_loader):
        ...
        cv2.rectangle(im2show, (bbox[4], bbox[5]), (bbox[6], bbox[7]), (255, 255, 0), 4)
